...
def get_job_status(
    ...
    while not completed:
        ...
        try:
            job_to_monitor = get_monitored_job(training_jobs["TrainingJobSummaries"], training_job_arn)
        except ValueError:
            return {"TrainingJobArn": training_job_arn, "Status": "Error: Found no job with arn %s" % training_job_arn}
        completed = job_to_monitor["TrainingJobStatus"] == "Completed"
        if time.time() - start_time > duration_threshold:
            return {"TrainingJobArn": training_job_arn, "Status": "Error: Exceeded maximum duration (%s s)" % duration_threshold}
